...
export class DeveloperMarketplaceService extends ScholarlyBaseService {
  ...
  async registerDeveloper(
    ...
    return this.withTiming('registerDeveloper', tenantId, async () => {
  ...
  async verifyDeveloper(tenantId: string, developerId: string, approved: boolean): Promise<Result<DeveloperAccount>> {
    try {
      Validator.tenantId(tenantId);
      Validator.required(developerId, 'developerId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('verifyDeveloper', tenantId, async () => {
      ...
      return this.repos.developerRepo.update(tenantId, developerId, {
  ...
  async createApp(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.developerId, 'developerId');
      Validator.required(data.name, 'name');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('createApp', tenantId, async () => {
  ...
  async submitAppForReview(tenantId: string, appId: string, version: AppVersion): Promise<Result<MarketplaceApp>> {
    try {
      Validator.tenantId(tenantId);
      Validator.required(appId, 'appId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('submitAppForReview', tenantId, async () => {
      const app = await this.repos.appRepo.findById(tenantId, appId);
      if (!app) throw new NotFoundError('App', appId);

      return this.repos.appRepo.update(tenantId, appId, {
  ...
  async reviewApp(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.appId, 'appId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('reviewApp', tenantId, async () => {
      ...
      return updated;
    }, { appId: data.appId, decision: data.decision });
  }
  ...
  async searchApps(
    ...
    return this.withTiming('searchApps', tenantId, async () => {
  ...
  async getApp(tenantId: string, appIdOrSlug: string): Promise<Result<MarketplaceApp>> {
    try {
      Validator.tenantId(tenantId);
      Validator.required(appIdOrSlug, 'appIdOrSlug');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('getApp', tenantId, async () => {
  ...
  async installApp(
    ...
    return this.withTiming('installApp', tenantId, async () => {
      const app = await this.repos.appRepo.findById(tenantId, data.appId);
      if (!app) throw new NotFoundError('App', data.appId);
  ...
  async submitAppReview(
    ...
    return this.withTiming('submitAppReview', tenantId, async () => {
      const app = await this.repos.appRepo.findById(tenantId, data.appId);
      if (!app) throw new NotFoundError('App', data.appId);
  ...
  async createCommunityRequest(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.requesterId, 'requesterId');
      Validator.required(data.title, 'title');
      if (data.fundingGoal <= BigInt(0)) throw new ValidationError('Funding goal must be positive');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('createCommunityRequest', tenantId, async () => {
  ...
  async pledgeToRequest(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.requestId, 'requestId');
      if (data.amount <= BigInt(0)) throw new ValidationError('Amount must be positive');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('pledgeToRequest', tenantId, async () => {
  ...
  async voteOnRequest(
    ...
    return this.withTiming('voteOnRequest', tenantId, async () => {
      ...
      return this.repos.requestRepo.update(tenantId, data.requestId, {
  ...
  async claimBounty(
    ...
    return this.withTiming('claimBounty', tenantId, async () => {
      const request = await this.repos.requestRepo.findById(tenantId, data.requestId);
      if (!request) throw new NotFoundError('Request', data.requestId);
      ...
      return claim;
    }, { requestId: data.requestId, developerId: data.developerId });
  }
  ...
  async acceptBountyClaim(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.claimId, 'claimId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('acceptBountyClaim', tenantId, async () => {
      const request = await this.repos.requestRepo.findById(tenantId, data.requestId);
      if (!request) throw new NotFoundError('Request', data.requestId);
      ...
      return updatedClaim;
    }, { requestId: data.requestId, claimId: data.claimId });
  }
  ...
  async approveMilestone(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.claimId, 'claimId');
      Validator.required(data.milestoneId, 'milestoneId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('approveMilestone', tenantId, async () => {
      const request = await this.repos.requestRepo.findById(tenantId, data.requestId);
      if (!request) throw new NotFoundError('Request', data.requestId);
      ...
      return { claim: updatedClaim, payoutAmount };
    }, { claimId: data.claimId, milestoneId: data.milestoneId });
  }
  ...
  async getCommunityRequests(
    ...
    return this.withTiming('getCommunityRequests', tenantId, async () => {
      const { limit = 20 } = options;
  ...
  async getDeveloperEarnings(tenantId: string, developerId: string): Promise<Result<{ totalEarned: TokenAmount; pendingPayout: TokenAmount; recentPayouts: DeveloperPayout[] }>> {
    ...
    return this.withTiming('getDeveloperEarnings', tenantId, async () => {
      const developer = await this.repos.developerRepo.findById(tenantId, developerId);
      if (!developer) throw new NotFoundError('Developer', developerId);
      ...
      return { totalEarned: developer.totalRevenue, pendingPayout, recentPayouts };
    }, { developerId });
  }
}

export default DeveloperMarketplaceService;
