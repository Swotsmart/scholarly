...
import { ASSESSMENT_EVENTS, createMeshEvent } from '../events/mesh-events';
...
export class AssessmentService extends ScholarlyBaseService {
  ...
  async createAssessment(
    ...
    try {
      ...
      Validator.required(data.schoolId, 'schoolId');
      Validator.required(data.title, 'title');
      Validator.required(data.purpose, 'purpose');
    ...
    return this.withTiming('createAssessment', tenantId, async () => {
      ...
      return saved;
    }, { schoolId: data.schoolId, purpose: data.purpose });
  }
  ...
  async getAssessment(tenantId: string, assessmentId: string): Promise<Result<AssessmentDefinition>> {
    ...
    return this.withTiming('getAssessment', tenantId, async () => {
      const cached = await this.cache.get<AssessmentDefinition>(
        this.cacheKey(tenantId, `assessment:${assessmentId}`)
      );
  ...
  async searchAssessments(
    ...
    return this.withTiming('searchAssessments', tenantId, async () => {
      return this.assessmentRepo.search(tenantId, query);
    }, { ...query });
  }
  ...
  async addSection(
    ...
    return this.withTiming('addSection', tenantId, async () => {
      const assessment = await this.assessmentRepo.findById(tenantId, assessmentId);
      if (!assessment) {
        throw new NotFoundError('Assessment', assessmentId);
      }
      ...
      await this.invalidateCache(tenantId, `assessment:${assessmentId}`);
  ...
  async addQuestion(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(assessmentId, 'assessmentId');
      Validator.required(sectionId, 'sectionId');
      Validator.required(data.stem, 'stem');
      Validator.positiveNumber(data.marks, 'marks');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('addQuestion', tenantId, async () => {
      const assessment = await this.assessmentRepo.findById(tenantId, assessmentId);
      if (!assessment) {
        throw new NotFoundError('Assessment', assessmentId);
      }
      ...
      return updated;
    }, { assessmentId, sectionId, questionType: data.type });
  }
  ...
  async publishAssessment(
    ...
    return this.withTiming('publishAssessment', tenantId, async () => {
      const assessment = await this.assessmentRepo.findById(tenantId, assessmentId);
      if (!assessment) {
        throw new NotFoundError('Assessment', assessmentId);
      }
  ...
  async startAttempt(
    ...
    return this.withTiming('startAttempt', tenantId, async () => {
      // Get assessment
  ...
  async saveResponse(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.attemptId, 'attemptId');
      Validator.required(data.questionId, 'questionId');
      Validator.required(data.studentId, 'studentId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('saveResponse', tenantId, async () => {
  ...
  async submitAttempt(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(attemptId, 'attemptId');
      Validator.required(studentId, 'studentId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('submitAttempt', tenantId, async () => {
  ...
  private async triggerAIMarkingAsync(
    ...
    try {
      ...
      this.logger.error('AI marking failed', error as Error, { attemptId });
    }
  }
  ...
  async markAttempt(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.attemptId, 'attemptId');
      Validator.required(data.markerId, 'markerId');
      Validator.required(data.responses, 'responses');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('markAttempt', tenantId, async () => {
      const attempt = await this.attemptRepo.findById(tenantId, data.attemptId);
      if (!attempt) {
        throw new NotFoundError('Attempt', data.attemptId);
      }
  ...
  async returnAttempt(
    ...
    return this.withTiming('returnAttempt', tenantId, async () => {
      const attempt = await this.attemptRepo.findById(tenantId, attemptId);
      if (!attempt) {
        throw new NotFoundError('Attempt', attemptId);
      }
  ...
  async generateAnalytics(
    ...
    return this.withTiming('generateAnalytics', tenantId, async () => {
      const assessment = await this.assessmentRepo.findById(tenantId, assessmentId);
      if (!assessment) {
        throw new NotFoundError('Assessment', assessmentId);
      }
  ...
  async assignPeerReviews(
    ...
    return this.withTiming('assignPeerReviews', tenantId, async () => {
      const assessment = await this.assessmentRepo.findById(tenantId, assessmentId);
