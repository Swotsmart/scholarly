...
import { ATTENDANCE_EVENTS } from '../events/mesh-events';
...
export class AttendanceService extends ScholarlyBaseService {
  ...
  async markAttendance(
    ...
    try {
      ...
      if (data.status === 'late' && !data.lateDetails) {
        throw new ValidationError('Late details required for late status');
      }
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('markAttendance', tenantId, async () => {
      ...
      return saved;
    }, { studentId: data.studentId, status: data.status });
  }
  ...
  async markClassAttendance(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.classGroup, 'classGroup');
      Validator.required(data.date, 'date');
      Validator.required(data.attendanceData, 'attendanceData');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('markClassAttendance', tenantId, async () => {
  ...
  async recordKioskCheckin(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.studentId, 'studentId');
      Validator.required(data.kioskId, 'kioskId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('recordKioskCheckin', tenantId, async () => {
      const student = await this.studentRepo.findById(tenantId, data.studentId);
      if (!student) {
        throw new NotFoundError('Student', data.studentId);
      }
  ...
  async submitParentAbsence(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(data.studentId, 'studentId');
      Validator.required(data.guardianId, 'guardianId');
      Validator.required(data.dates, 'dates');
      Validator.required(data.reason, 'reason');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('submitParentAbsence', tenantId, async () => {
      const records: AttendanceRecord[] = [];
      ...
      return records;
    }, { studentId: data.studentId, dates: data.dates.length });
  }
  ...
  async detectPatterns(
    ...
    return this.withTiming('detectPatterns', tenantId, async () => {
  ...
  private async detectPatternsAsync(tenantId: string, studentId: string): Promise<void> {
    try {
      ...
      this.logger.error('Pattern detection failed', error as Error, { studentId });
    }
  }
  ...
  async acknowledgeAlert(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(alertId, 'alertId');
    } catch (e) {
      return failure(e as ValidationError);
    }
  ...
  async resolveAlert(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(alertId, 'alertId');
      Validator.required(data.action, 'action');
      Validator.required(data.outcome, 'outcome');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('resolveAlert', tenantId, async () => {
      const alert = await this.alertRepo.findById(tenantId, alertId);
      if (!alert) {
        throw new NotFoundError('Alert', alertId);
      }
  ...
  async calculateChronicAbsenteeismRiskScore(
    ...
    return this.withTiming('calculateChronicAbsenteeismRiskScore', tenantId, async () => {
  ...
  async getStudentAttendanceSummary(
    ...
    return this.withTiming('getStudentAttendanceSummary', tenantId, async () => {
  ...
  async getClassAttendanceReport(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(classGroup, 'classGroup');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('getClassAttendanceReport', tenantId, async () => {
    ...
    }, { classGroup, date: date.toISOString() });
  }
}
