...
import { EnrollmentApplication } from '../shared/mesh-types';
...
export class FormBuilderService extends ScholarlyBaseService {
  ...
  async createFormConfig(
    ...
    return this.withTiming('createFormConfig', tenantId, async () => {
      const now = new Date();
  ...
  async createFromTemplate(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(templateId, 'templateId');
      Validator.required(data.name, 'name');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('createFromTemplate', tenantId, async () => {
  ...
  async updateFormConfig(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(formId, 'formId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('updateFormConfig', tenantId, async () => {
  ...
  async addSection(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(formId, 'formId');
      Validator.required(section.title, 'section.title');
    } catch (e) {
      return failure(e as ValidationError);
    }
    ...
    }, { formId, sectionTitle: section.title });
  }
  ...
  async addField(
    ...
    try {
      ...
      Validator.required(field.type, 'field.type');
      Validator.required(field.label, 'field.label');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('addField', tenantId, async () => {
      const config = await this.configRepo.findById(tenantId, formId);
      if (!config) {
        throw new NotFoundError('EnrollmentFormConfig', formId);
      }
    ...
    }, { formId, sectionId, fieldLabel: field.label });
  }
  ...
  async updateField(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(formId, 'formId');
      Validator.required(fieldId, 'fieldId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('updateField', tenantId, async () => {
      const config = await this.configRepo.findById(tenantId, formId);
      if (!config) {
        throw new NotFoundError('EnrollmentFormConfig', formId);
      }
  ...
  async removeField(
    ...
    return this.withTiming('removeField', tenantId, async () => {
      const config = await this.configRepo.findById(tenantId, formId);
      if (!config) {
        throw new NotFoundError('EnrollmentFormConfig', formId);
      }
  ...
  async publishForm(
    ...
    return this.withTiming('publishForm', tenantId, async () => {
      const config = await this.configRepo.findById(tenantId, formId);
      if (!config) {
        throw new NotFoundError('EnrollmentFormConfig', formId);
      }
  ...
  async getActiveForm(
    ...
    return this.withTiming('getActiveForm', tenantId, async () => {
      return this.configRepo.findActive(tenantId, yearLevel, enrollmentType);
    }, { yearLevel, enrollmentType });
  }
  ...
  async startSubmission(
    ...
    try {
      Validator.tenantId(tenantId);
      Validator.required(formConfigId, 'formConfigId');
    } catch (e) {
      return failure(e as ValidationError);
    }

    return this.withTiming('startSubmission', tenantId, async () => {
  ...
  async saveResponses(
    ...
    return this.withTiming('saveResponses', tenantId, async () => {
      ...
      return updated;
    }, { submissionId, responseCount: responses.length });
  }
  ...
  async submitForm(
    ...
    return this.withTiming('submitForm', tenantId, async () => {
      const submission = await this.submissionRepo.findById(tenantId, submissionId);
      if (!submission) {
        throw new NotFoundError('FormSubmission', submissionId);
      }
  ...
  validateResponses(
    ...
    for (const rule of config.validationRules) {
      try {
        ...
        this.logger.warn(`Cross-field validation rule ${rule.id} failed to evaluate`, { error: e });
      }
    }
  ...
  private validateField(
    ...
    if (field.validation.customValidation) {
      try {
        ...
        this.logger.warn(`Custom validation for field ${field.id} failed`, { error: e });
      }
    }
