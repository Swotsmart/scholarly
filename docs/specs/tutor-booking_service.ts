...
import {
  ...
  AuthorizationError, Validator, EventBus, Cache, ScholarlyConfig, TutorUser,
  LearnerUser, TutoringSession, SessionStatus, SessionType, SessionFeedback,
  TutorPricing, SafeguardingCheck, Jurisdiction, JURISDICTION_REQUIREMENTS
} from '../shared/types';
...
export interface TutorMatch {
  ...
  matchScore: number;
  matchReasons: string[];
  matchBreakdown: { subjectMatch: number; styleMatch: number; trustMatch: number; priceMatch: number };
...
export class TutorBookingService extends ScholarlyBaseService {
  ...
  constructor(deps: {
    ...
    tutorRepo: TutorRepository; learnerRepo: LearnerRepository;
    sessionRepo: SessionRepository; bookingRepo: BookingRepository;
  }) {
   ...
   */
  async findTutors(tenantId: string, request: TutorMatchRequest): Promise<Result<{ tutors: TutorMatch[]; totalCount: number }>> {
    try {
    ...
    return this.withTiming('findTutors', tenantId, async () => {
      ...
      await this.publishEvent('scholarly.tutor.search_completed', tenantId, {
        learnerId: request.learnerId, subjectId: request.subjectId, resultsCount: matches.length
      });
   ...
   */
  async getProactiveSuggestions(tenantId: string, learnerId: string): Promise<Result<{
    areasNeedingSupport: string[];
    schoolTeacherNote: string;
    marketplaceSuggestion: string;
  }>> {
    try { Validator.tenantId(tenantId); Validator.userId(learnerId); } 
    catch (e) { return failure(e as ValidationError); }

    return this.withTiming('getProactiveSuggestions', tenantId, async () => {
      // Would integrate with LIS - for now return structure
      return {
        areasNeedingSupport: ['Fractions', 'Word problems'],
        schoolTeacherNote: `We've noticed some challenges with fractions and word problems. ` +
          `Consider discussing with their classroom teacher for additional school support.`,
        marketplaceSuggestion: `If school support isn't sufficient, our tutor marketplace ` +
   ...
   */
  async createBooking(tenantId: string, bookedByUserId: string, request: {
    tutorId: string; learnerIds: string[]; scheduledStart: Date; duration: number;
    sessionType: SessionType; subjectId: string; topicsNeedingHelp: string[];
    learnerNotes?: string; openToOthers?: boolean; maxGroupSize?: number;
    ...
    return this.withTiming('createBooking', tenantId, async () => {
      const tutor = await this.tutorRepo.findById(tenantId, request.tutorId);
      if (!tutor) throw new NotFoundError('Tutor', request.tutorId);

      // Verify availability
      const isAvailable = await this.checkAvailability(tenantId, request.tutorId, request.scheduledStart, request.duration);
      if (!isAvailable) throw new ValidationError('Tutor not available at requested time');

      // Calculate pricing
      const pricing = this.calculatePricing(tutor.pricing, request.duration, request.learnerIds.length);

      const booking: Booking = {
        id: this.generateId('booking'), tenantId, tutorId: request.tutorId,
        learnerIds: request.learnerIds, bookedByUserId,
        scheduledStart: request.scheduledStart,
        scheduledEnd: new Date(request.scheduledStart.getTime() + request.duration * 60000),
        sessionType: request.sessionType, subjectId: request.subjectId,
        topicsNeedingHelp: request.topicsNeedingHelp, learnerNotes: request.learnerNotes,
        isGroupSession: request.learnerIds.length > 1,
        openToOthers: request.openToOthers || false,
        maxGroupSize: request.maxGroupSize || 1,
        pricing, status: 'pending', createdAt: new Date()
      };
      ...
      await this.publishEvent('scholarly.booking.created', tenantId, {
        bookingId: savedBooking.id, tutorId: request.tutorId, scheduledStart: request.scheduledStart
      });
   ...
   */
  async confirmBooking(tenantId: string, bookingId: string, tutorId: string): Promise<Result<Booking>> {
    try { Validator.tenantId(tenantId); Validator.required(bookingId, 'bookingId'); } 
    catch (e) { return failure(e as ValidationError); }

    return this.withTiming('confirmBooking', tenantId, async () => {
      const booking = await this.bookingRepo.findById(tenantId, bookingId);
      if (!booking) throw new NotFoundError('Booking', bookingId);
      if (booking.tutorId !== tutorId) throw new AuthorizationError('Only assigned tutor can confirm');
   ...
   * Cancel a booking
   */
  async cancelBooking(tenantId: string, bookingId: string, userId: string, reason: string): Promise<Result<Booking>> {
    try { Validator.tenantId(tenantId); Validator.required(bookingId, 'bookingId'); } 
    catch (e) { return failure(e as ValidationError); }

    return this.withTiming('cancelBooking', tenantId, async () => {
      const booking = await this.bookingRepo.findById(tenantId, bookingId);
      if (!booking) throw new NotFoundError('Booking', bookingId);
      ...
      await this.publishEvent('scholarly.booking.cancelled', tenantId, { bookingId, userId, reason, refundAmount });
      return updated;
    }, { bookingId });
  }
  ...
  async getUpcomingSessions(tenantId: string, userId: string, limit = 10): Promise<Result<TutoringSession[]>> {
    try { Validator.tenantId(tenantId); Validator.userId(userId); } 
    catch (e) { return failure(e as ValidationError); }
    return this.withTiming('getUpcomingSessions', tenantId, async () => {
  ...
  async startSession(tenantId: string, sessionId: string): Promise<Result<TutoringSession>> {
    try { Validator.tenantId(tenantId); Validator.required(sessionId, 'sessionId'); } 
    catch (e) { return failure(e as ValidationError); }
  ...
  async completeSession(tenantId: string, sessionId: string, data: {
    sessionNotes?: string; homeworkAssigned?: string;
    ...
    return this.withTiming('completeSession', tenantId, async () => {
      ...
      await this.publishEvent('scholarly.session.completed', tenantId, { sessionId, tutorEarnings: session.tutorEarnings });
      return { session: updated, tutorEarnings: session.tutorEarnings, tokenRewards };
    }, { sessionId });
  }

  async submitFeedback(tenantId: string, sessionId: string, userId: string, feedback: SessionFeedback): Promise<Result<TutoringSession>> {
    try { Validator.tenantId(tenantId); Validator.required(sessionId, 'sessionId'); } 
    catch (e) { return failure(e as ValidationError); }

    return this.withTiming('submitFeedback', tenantId, async () => {
      const session = await this.sessionRepo.findById(tenantId, sessionId);
      if (!session) throw new NotFoundError('Session', sessionId);
      ...
      const updated = await this.sessionRepo.update(tenantId, sessionId, session);
      if (userId !== session.tutorId) await this.updateTutorRating(tenantId, session.tutorId, feedback.rating);
      return updated;
    }, { sessionId });
  }
  ...
  async startProfileBuilder(tenantId: string, tutorId: string): Promise<Result<ProfileBuilderSession>> {
    try { Validator.tenantId(tenantId); Validator.userId(tutorId); } 
    catch (e) { return failure(e as ValidationError); }

    return this.withTiming('startProfileBuilder', tenantId, async () => ({
      id: this.generateId('profile_builder'), tutorId, currentStep: 0,
      totalSteps: PROFILE_QUESTIONS.length, responses: []
    }), { tutorId });
  }

  async answerProfileQuestion(tenantId: string, sessionId: string, answer: string): Promise<Result<{
    ...
    return this.withTiming('answerProfileQuestion', tenantId, async () => {
  ...
  private verifySafeguarding(tutor: TutorUser, jurisdiction: Jurisdiction): boolean {
    ...
    return tutor.safeguardingChecks.some(c => 
      c.type === req.safeguardingCheckType && c.status === 'valid' && 
      (!c.expiresAt || c.expiresAt > new Date())
    );
  }

  private calculateMatch(tutor: TutorUser, learner: LearnerUser, request: TutorMatchRequest): TutorMatch {
    const subjectMatch = tutor.subjects.find(s => s.subjectId === request.subjectId)?.confidenceLevel || 0;
    ...
    const priceMatch = request.maxHourlyRate 
      ? (tutor.pricing.hourlyRate1to1 <= request.maxHourlyRate ? 100 : 50) : 80;

    const matchScore = Math.round(subjectMatch * 0.35 + styleMatch * 0.25 + trustMatch * 0.25 + priceMatch * 0.15);
    ...
    return {
      tutor, matchScore, matchReasons,
      matchBreakdown: { subjectMatch: subjectMatch * 20, styleMatch, trustMatch, priceMatch },
      availableSlots: [], estimatedPrice: tutor.pricing.hourlyRate1to1
    };
  }

  private async checkAvailability(tenantId: string, tutorId: string, start: Date, duration: number): Promise<boolean> {
    const end = new Date(start.getTime() + duration * 60000);
    const sessions = await this.sessionRepo.findByTutor(tenantId, tutorId, { start, end });
    return sessions.length === 0;
  }

  private calculatePricing(tutorPricing: TutorPricing, duration: number, studentCount: number): BookingPricing {
    const hours = duration / 60;
    const baseRate = studentCount === 1 ? tutorPricing.hourlyRate1to1 : tutorPricing.hourlyRateGroup;
    ...
    return {
      baseRate, duration, groupDiscount, subtotal: discounted,
      platformFee, total: discounted, currency: tutorPricing.currency,
      tutorEarnings: discounted - platformFee
    };
  }

  private async createSession(tenantId: string, booking: Booking, tutor: TutorUser): Promise<TutoringSession> {
    const session: TutoringSession = {
      id: this.generateId('session'), tenantId, tutorId: booking.tutorId,
      learnerIds: booking.learnerIds, parentIds: [],
      scheduledStart: booking.scheduledStart, scheduledEnd: booking.scheduledEnd,
      timezone: tutor.availability.timezone, sessionType: booking.sessionType,
      isGroupSession: booking.isGroupSession, subjectId: booking.subjectId,
      subjectName: tutor.subjects.find(s => s.subjectId === booking.subjectId)?.subjectName || '',
      topicsFocus: booking.topicsNeedingHelp, status: SessionStatus.SCHEDULED,
      bookingId: booking.id, resourcesShared: [], billingStatus: 'pending',
      amountCharged: booking.pricing.total, tutorEarnings: booking.pricing.tutorEarnings,
      platformCommission: booking.pricing.platformFee, tokenRewards: 0
    };
  ...
  private async updateTutorRating(tenantId: string, tutorId: string, rating: number): Promise<void> {
  ...
  private extractKeywords(text: string): string[] {
    ...
    return text.toLowerCase().split(/\s+/).filter(w => w.length > 3 && !stops.includes(w)).slice(0, 10);
  }

  private generateBio(keywords: string[]): string {
    return `Passionate educator specializing in ${keywords.slice(0, 3).join(', ')}. ` +
           `Focused on making learning engaging and accessible for every student.`;
  }
}

const PROFILE_QUESTIONS = [
  { id: 'pb_1', question: 'What subjects do you teach and what makes you passionate about them?', helpText: 'Share your enthusiasm!' },
  { id: 'pb_2', question: 'How would you describe your teaching style?', helpText: 'Patient? Energetic? Hands-on?' },
  { id: 'pb_3', question: 'What age groups do you work best with?', helpText: 'Primary, secondary, adult?' },
  { id: 'pb_4', question: 'Describe a teaching moment you are proud of', helpText: 'A specific example helps parents' },
  { id: 'pb_5', question: 'How do you handle students who are struggling?', helpText: 'Parents want to know you can handle difficulty' }
];

export { TutorBookingService };
