...
import { AnalyticsReportingService, getAnalyticsReportingService } from './analytics-reporting.service';
...
export class EarlyYearsService extends ScholarlyBaseService {
  ...
  constructor(deps: {
    ...
    this.analyticsService = getAnalyticsReportingService();
  }
  ...
  async createLearner(
    ...
    try {
      ...
      // Save to database
      await prisma.earlyYearsChild.create({
        data: learner as any,
      });

      // Data Lake - learner.created event (via streaming pipeline)
      ...
      return success(learner as EarlyYearsLearner);
    } catch (error) {
      log.error('Failed to create early years learner', error as Error);
  ...
  async getLearner(
    ...
    try {
      const learner = await prisma.earlyYearsChild.findFirst({
  ...
  async startCharacterConversation(
    ...
    try {
      ...
      // Data Lake - character_conversation.started event (via streaming pipeline)

      return success(conversation);
    } catch (error) {
      log.error('Failed to start character conversation', error as Error);
      return failure(error as Error);
    }
  }
  ...
  async sendCharacterMessage(
    ...
    try {
      ...
      // Get AI response
      const aiResponse = await this.aiService.complete(tenantId, {
        messages: aiMessages,
      ...
      // Data Lake - character_conversation.message event (via streaming pipeline)

      return success(assistantMessage);
    } catch (error) {
      log.error('Failed to send character message', error as Error);
      return failure(error as Error);
    }
  }
  ...
  "activities": [
    {
      ...
      // Data Lake - phonics.activities_generated event (via streaming pipeline)

      return success(activities);
    } catch (error) {
      log.error('Failed to generate phonics activities', error as Error);
      return failure(error as Error);
    }
  }
...
Return as JSON:
{
  ...
  "phonemesPracticed": ["s", "a", "t"]
      ...
      log.error('Failed to generate decodable reader', error as Error);
      return failure(error as Error);
    }
  }
  ...
  async recordPhonicsSession(
    ...
    try {
      // Data Lake - phonics.session_completed and phonics.activity_completed events (via streaming pipeline)
      ...
      log.error('Failed to record phonics session', error as Error);
      return failure(error as Error);
    }
  }
  ...
  async recordEyeTrackingSession(
    ...
    try {
      // Data Lake - eye_tracking.session_completed and eye_tracking.concern_detected events (via streaming pipeline)
      ...
      log.error('Failed to record eye tracking session', error as Error);
      return failure(error as Error);
    }
  }
  ...
  async predictReadingDifficulty(
    ...
    try {
      ...
      // Map risk level (ML might return 'critical', map to 'high')
      const mappedRiskLevel = mlPrediction.riskLevel === 'critical' ? 'high' : mlPrediction.riskLevel;

      // Enrich with reading-specific factors
      const prediction: ReadingDifficultyPrediction = {
        learnerId,
        riskScore: mlPrediction.riskScore,
        riskLevel: mappedRiskLevel as 'high' | 'medium' | 'low',
      ...
      // Data Lake - ml.reading_difficulty_predicted event (via streaming pipeline)

      return success(prediction);
    } catch (error) {
      log.error('Failed to predict reading difficulty', error as Error);
      return failure(error as Error);
    }
  }
  ...
  async getParentDashboard(
    ...
    try {
      ...
      // Dashboard view tracking would go to Data Lake via streaming pipeline

      return success(dashboard);
    } catch (error) {
      log.error('Failed to get parent dashboard', error as Error);
      return failure(error as Error);
    }
  }
  ...
  async registerDashboardWidgets(tenantId: string): Promise<Result<void>> {
    // Widget registration would be handled through AnalyticsReportingService dashboard configuration
    // Widgets: early_years_phonics_progress, early_years_numeracy_progress, early_years_weekly_activity,
    // early_years_story_progress, early_years_home_activities
    void tenantId; // Acknowledge parameter for future use
    return success(undefined);
  }
  ...
  async createPicturePassword(
    ...
    try {
      ...
      await prisma.earlyYearsPicturePassword.create({ data: password as any });
  ...
  async verifyPicturePassword(
    ...
    try {
      const password = await prisma.earlyYearsPicturePassword.findFirst({
      ...
      const isCorrect = attemptHash === password.imageSequenceHash;

      // Record attempt
      await prisma.earlyYearsPicturePasswordAttempt.create({
        data: {
          ...
          userAgent: metadata?.deviceFingerprint,
        },
      });

      if (isCorrect) {
        // Reset failed attempts
        await prisma.earlyYearsPicturePassword.update({
...
- Current activity: ${context?.currentActivity || 'free conversation'}`;
  ...
  private async saveConversation(tenantId: string, conversation: EarlyYearsConversation): Promise<void> {
    void tenantId; // Tenant isolation via learnerId relationship
    const conversationData = conversation as unknown as Record<string, unknown>;
    await prisma.earlyYearsConversation.upsert({
      where: { id: conversation.id },
      create: conversation as any,
      update: {
        messages: conversation.messages as any,
        endedAt: conversationData.endedAt as Date | undefined,
        isActive: (conversationData.isActive as boolean) ?? true,
      },
    });
  }
