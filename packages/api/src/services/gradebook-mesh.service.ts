...
} from './gradebook-mesh-types';

import { GRADEBOOK_EVENTS, createMeshEvent } from './mesh-events';
...
export class GradebookMeshService extends ScholarlyBaseService {
  ...
  async createGradebook(
    ...
    if (!data.classId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'classId is required' });
    }
    if (!data.subject) {
      return failure({ code: 'VALIDATION_ERROR', message: 'subject is required' });
    }
    if (!data.teacherId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'teacherId is required' });
    }
    if (!data.gradingPolicyId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'gradingPolicyId is required' });
    }

    return this.withTiming('createGradebook', async () => {
  ...
  async getGradebook(tenantId: string, gradebookId: string): Promise<Result<Gradebook>> {
    ...
    if (!gradebookId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'gradebookId is required' });
    }

    return this.withTiming('getGradebook', async () => {
  ...
  async getTeacherGradebooks(tenantId: string, teacherId: string): Promise<Result<Gradebook[]>> {
    ...
    return this.withTiming('getTeacherGradebooks', async () => {
      const result = await this.gradebookRepo.findByTeacher(tenantId, teacherId);
  ...
  async addItem(
    ...
    if (!data.categoryId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'categoryId is required' });
    }
    if (!data.title) {
      return failure({ code: 'VALIDATION_ERROR', message: 'title is required' });
    }
    if (!Validator.isPositiveNumber(data.maxPoints)) {
      return failure({ code: 'VALIDATION_ERROR', message: 'maxPoints must be a positive number' });
    }

    return this.withTiming('addItem', async () => {
  ...
  async linkAssessment(
    ...
    if (!itemId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'itemId is required' });
    }
    if (!assessmentId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'assessmentId is required' });
    }

    return this.withTiming('linkAssessment', async () => {
  ...
  async enterScore(
    ...
    return this.withTiming('enterScore', async () => {
  ...
  async excuseScore(
    ...
    return this.withTiming('excuseScore', async () => {
      const item = await this.itemRepo.findById(tenantId, itemId);
      if (!item) {
        throw new NotFoundError('GradebookItem', itemId);
      }
  ...
  async calculateStudentGrade(
    ...
    return this.withTiming('calculateStudentGrade', async () => {
      const gradebook = await this.gradebookRepo.findById(tenantId, gradebookId);
      if (!gradebook) {
        throw new NotFoundError('Gradebook', gradebookId);
      }
  ...
  async calculateAllGrades(
    ...
    return this.withTiming('calculateAllGrades', async () => {
      const gradebook = await this.gradebookRepo.findById(tenantId, gradebookId);
      if (!gradebook) {
        throw new NotFoundError('Gradebook', gradebookId);
      }
  ...
  async generateReportCards(
    ...
    if (!data.periodId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'periodId is required' });
    }
    if (!data.templateId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'templateId is required' });
    }
    if (!data.studentIds) {
      return failure({ code: 'VALIDATION_ERROR', message: 'studentIds is required' });
    }

    return this.withTiming('generateReportCards', async () => {
  ...
  async approveNarrative(
    ...
    if (!reportId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'reportId is required' });
    }
    if (!narrativeId) {
      return failure({ code: 'VALIDATION_ERROR', message: 'narrativeId is required' });
    }
    if (!editedContent) {
      return failure({ code: 'VALIDATION_ERROR', message: 'editedContent is required' });
    }

    return this.withTiming('approveNarrative', async () => {
  ...
  async publishReport(
    ...
    return this.withTiming('publishReport', async () => {
      const report = await this.reportRepo.findById(tenantId, reportId);
      if (!report) {
        throw new NotFoundError('ReportCard', reportId);
      }
  ...
  private async checkGradeAlerts(tenantId: string, gradebookId: string, studentId: string): Promise<void> {
    try {
      ...
      log.error('Grade alert check failed', error as Error, { gradebookId, studentId });
    }
  }
}
...
let gradebookMeshServiceInstance: GradebookMeshService | null = null;

export function initializeGradebookMeshService(deps: {
  gradebookRepo: GradebookRepository;
  itemRepo: GradebookItemRepository;
  policyRepo: GradingPolicyRepository;
  reportRepo: ReportCardRepository;
  ...
  gradebookMeshServiceInstance = new GradebookMeshService(
    deps.gradebookRepo,
    deps.itemRepo,
    deps.policyRepo,
    deps.reportRepo,
  ...
  return gradebookMeshServiceInstance;
}

export function getGradebookMeshService(): GradebookMeshService {
  if (!gradebookMeshServiceInstance) {
    throw new Error('GradebookMeshService has not been initialized. Call initializeGradebookMeshService first.');
  }
  return gradebookMeshServiceInstance;
}
