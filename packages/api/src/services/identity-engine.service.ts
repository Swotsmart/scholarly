...
export function initializeIdentityService(
  ...
  identityServiceInstance = new IdentityService(repo, eventBus, otpService, logger, config);
  return identityServiceInstance;
}

export function getIdentityService(): IdentityService {
  if (!identityServiceInstance) {
    throw new Error('IdentityService not initialized. Call initializeIdentityService first.');
  }
  return identityServiceInstance;
}
