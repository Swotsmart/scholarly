...
import { Result, success, failure, IdentityError, IdentityNotFoundError } from './identity-engine.service';
import { ProviderRegistry } from './kyc-engine.service';
...
export class KybService {
  constructor(
    ...
    private config: KybServiceConfig
  ) {}
  ...
  async calculateKybLevel(
    ...
    try {
      ...
      // Update business
      await this.repo.updateBusiness(businessId, {
        kybLevel: level as BusinessIdentity['kybLevel'],
...
let kybServiceInstance: KybService | null = null;

export function initializeKybService(
  repo: IdentityRepository,
  providers: ProviderRegistry,
  eventBus: EventBus,
  logger: Logger,
  config: KybServiceConfig
): KybService {
  kybServiceInstance = new KybService(repo, providers, eventBus, logger, config);
  return kybServiceInstance;
}

export function getKybService(): KybService {
  if (!kybServiceInstance) {
    throw new Error('KybService not initialized. Call initializeKybService first.');
  }
  return kybServiceInstance;
}
