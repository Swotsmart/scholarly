...
export class LanguageLearningService extends ScholarlyBaseService {
  ...
  async createLearner(
    ...
    try {
      ...
      await prisma.languageLearnerProfile.create({ data: learner as any });
      ...
      log.error('Failed to create language learner', error as Error);
  ...
  async getLearner(tenantId: string, learnerId: string): Promise<Result<LanguageLearner>> {
    try {
      const learner = await prisma.languageLearnerProfile.findFirst({
  ...
  async startConversation(
    ...
    try {
      ...
      if (isFailure(learnerResult)) return failure(learnerResult.error);
  ...
  async endConversation(tenantId: string, conversationId: string): Promise<Result<ConversationSummary>> {
    try {
      ...
      await prisma.languageLearnerProfile.update({
        where: { id: conversation.learnerId },
        data: {
          totalSpeakingMinutes: { increment: conversation.durationMinutes || 0 },
...
Return JSON:
{
  ...
  async createSRSCard(
    ...
    try {
      ...
      await prisma.languageVocabularyItem.create({ data: newCard as any });
  ...
  async recordSRSReview(
    ...
    try {
      const card = await prisma.languageVocabularyItem.findFirst({
        where: {
          id: cardId,
          progress: { profile: { tenantId } }
        },
      ...
      await prisma.languageVocabularyItem.update({ where: { id: cardId }, data: updatedData as any });
      ...
      await prisma.languageVocabularyReview.create({ data: review as any });
  ...
  async getDueCards(tenantId: string, learnerId: string, limit: number = 20): Promise<Result<SRSCard[]>> {
    try {
      const cards = await prisma.languageVocabularyItem.findMany({
        where: {
          progress: {
            profile: { id: learnerId, tenantId }
          },
          nextReviewAt: { lte: new Date() },
        },
        orderBy: [{ repetitions: 'asc' }, { nextReviewAt: 'asc' }],
        take: limit,
      });
  ...
  async getSRSStatistics(tenantId: string, learnerId: string): Promise<Result<SRSStatistics>> {
    try {
      ...
      const cards = await prisma.languageVocabularyItem.findMany({
        where: {
          progress: { profile: { id: learnerId, tenantId } }
        },
      }) as unknown as SRSCard[];
  ...
  async predictProficiency(tenantId: string, learnerId: string): Promise<Result<ProficiencyPrediction>> {
    try {
      ...
      // Get ML prediction
      const mlResult = await this.mlPipelineService.predictPerformance(tenantId, learnerId, 'student', 'language_proficiency');
  ...
  async conductPlacementTest(tenantId: string, learnerId: string, language: LanguageCode): Promise<Result<PlacementTestResult>> {
    try {
      ...
      await prisma.languageLearnerProfile.update({
        where: { id: learnerId },
        data: {
          overallLevel: result.recommendedCEFR,
          readingLevel: result.skillBreakdown.reading,
          writingLevel: result.skillBreakdown.writing,
          listeningLevel: result.skillBreakdown.listening,
          speakingLevel: result.skillBreakdown.speaking,
          lastActiveAt: new Date(),
        },
      });
  ...
  async registerDashboardWidgets(tenantId: string): Promise<Result<void>> {
    ...
    // Widgets: language_cefr_radar, language_vocabulary_progress, language_srs_due,
    // language_conversation_time, language_level_progress
  ...
  private async saveConversation(tenantId: string, conversation: LanguageConversation): Promise<void> {
    void tenantId; // Tenant isolation via profileId relationship
    const metrics = conversation.metrics as unknown as {
      fluencyScore?: number;
    ...
    await prisma.languageConversation.upsert({
      ...
      update: {
        ...
        vocabularyUsed: metrics?.newVocabularyIntroduced || [],
      },
    });
  }

  private async getConversationById(tenantId: string, id: string): Promise<LanguageConversation | null> {
    const conv = await prisma.languageConversation.findFirst({
      where: {
        id,
        profile: { tenantId }
      }
