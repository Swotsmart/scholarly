...
export interface TokenWalletProvider {
...
export class TokenEconomyService extends ScholarlyBaseService {
  constructor(
    ...
    private readonly providers: {
      ...
      walletProvider: TokenWalletProvider;
    }
  ) {
  ...
  async getUserBalance(tenantId: string, userId: string): Promise<Result<UserTokenBalance>> {
    ...
    return this.withTiming('getUserBalance', async () => {
      const wallet = await this.providers.walletProvider.getConnectedWallet(userId);
      if (!wallet) {
        return success<UserTokenBalance>({
          userId,
  ...
  async transferTokens(
    ...
    if (!data.fromUserId) {
      return failure(new ValidationError('fromUserId is required'));
    }
    if (data.amount <= BigInt(0)) {
      return failure(new ValidationError('Amount must be positive'));
    }

    return this.withTiming('transferTokens', async () => {
  ...
  async getStakingPools(tenantId: string, purpose?: StakingPurpose): Promise<Result<StakingPool[]>> {
    ...
    return this.withTiming('getStakingPools', async () => {
      const pools = purpose
        ? await this.repos.poolRepo.findByPurpose(tenantId, purpose)
        : await this.repos.poolRepo.findAll(tenantId, true);
  ...
  async stakeTokens(
    ...
    if (!data.poolId) {
      return failure(new ValidationError('poolId is required'));
    }
    if (data.amount <= BigInt(0)) {
      return failure(new ValidationError('Amount must be positive'));
    }

    return this.withTiming('stakeTokens', async () => {
  ...
  async unstakeTokens(
    ...
    if (!data.positionId) {
      return failure(new ValidationError('positionId is required'));
    }

    return this.withTiming('unstakeTokens', async () => {
      ...
      const updated = await this.repos.positionRepo.update(tenantId, data.positionId, {
        status: 'unstaking',
  ...
  async issueReward(
    ...
    if (!data.recipientId) {
      return failure(new ValidationError('recipientId is required'));
    }
    if (data.amount <= BigInt(0)) {
      return failure(new ValidationError('Amount must be positive'));
    }

    return this.withTiming('issueReward', async () => {
  ...
  async claimRewards(tenantId: string, userId: string): Promise<Result<{ totalAmount: TokenAmount; txHash: TxHash }>> {
    ...
    return this.withTiming('claimRewards', async () => {
      const wallet = await this.providers.walletProvider.getConnectedWallet(userId);
      if (!wallet) throw new AuthorizationError('No wallet connected');
      ...
      return success({ totalAmount, txHash });
    });
  }
  ...
  async mintPublisherNFT(
    ...
    if (!data.creatorId) {
      return failure(new ValidationError('creatorId is required'));
    }
    if (!data.contentId) {
      return failure(new ValidationError('contentId is required'));
    }
    if (data.royaltyPercent < 0 || data.royaltyPercent > 25) {
      return failure(new ValidationError('Royalty must be 0-25%'));
    }

    return this.withTiming('mintPublisherNFT', async () => {
  ...
  async submitForValidation(tenantId: string, tokenId: bigint): Promise<Result<{ nft: PublisherNFT; validators: ContentValidator[] }>> {
    ...
    return this.withTiming('submitForValidation', async () => {
      ...
      return success({ nft: updated, validators });
    });
  }
  ...
  async submitValidationScore(
    ...
    if (!data.validatorId) {
      return failure(new ValidationError('validatorId is required'));
    }

    return this.withTiming('submitValidationScore', async () => {
      ...
      let newStatus: NFTValidationStatus = nft.validationStatus;
      ...
      const updated = await this.repos.nftRepo.update(tenantId, data.tokenId, { validatorScores: updatedScores, validationStatus: newStatus });
      return success(updated);
    });
  }
  ...
  async listNFTForSale(tenantId: string, data: { ownerId: string; tokenId: bigint; price: TokenAmount }): Promise<Result<PublisherNFT>> {
    ...
    if (!data.ownerId) {
      return failure(new ValidationError('ownerId is required'));
    }
    if (data.price <= BigInt(0)) {
      return failure(new ValidationError('Price must be positive'));
    }

    return this.withTiming('listNFTForSale', async () => {
      const nft = await this.repos.nftRepo.findByTokenId(tenantId, data.tokenId);
      if (!nft) throw new NotFoundError('NFT', data.tokenId.toString());
      ...
      const updated = await this.repos.nftRepo.update(tenantId, data.tokenId, { isListed: true, listPrice: data.price });
      return success(updated);
    });
  }
  ...
  async purchaseNFT(tenantId: string, data: { buyerId: string; tokenId: bigint }): Promise<Result<NFTPurchase>> {
    ...
    if (!data.buyerId) {
      return failure(new ValidationError('buyerId is required'));
    }

    return this.withTiming('purchaseNFT', async () => {
      const nft = await this.repos.nftRepo.findByTokenId(tenantId, data.tokenId);
      if (!nft) throw new NotFoundError('NFT', data.tokenId.toString());
  ...
  async registerAsValidator(
    ...
    if (data.expertiseAreas.length === 0) {
      return failure(new ValidationError('Expertise required'));
    }

    return this.withTiming('registerAsValidator', async () => {
      const wallet = await this.providers.walletProvider.getConnectedWallet(data.userId);
      if (!wallet) throw new AuthorizationError('No wallet connected');
  ...
  async slashValidator(
    ...
    return this.withTiming('slashValidator', async () => {
      const validator = await this.repos.validatorRepo.findById(tenantId, data.validatorId);
      if (!validator) throw new NotFoundError('Validator', data.validatorId);
...
let instance: TokenEconomyService | null = null;

export function initializeTokenEconomyService(deps?: any): TokenEconomyService {
  if (!instance) {
    instance = new TokenEconomyService(deps, deps?.repos || {}, deps?.providers || {});
    log.info('TokenEconomyService initialized');
  }
  return instance;
}

export function getTokenEconomyService(): TokenEconomyService {
  if (!instance) {
    throw new Error('TokenEconomyService not initialized. Call initializeTokenEconomyService() first.');
  }
  return instance;
}
