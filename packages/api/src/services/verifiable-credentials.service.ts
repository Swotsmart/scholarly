...
export class VerifiableCredentialsService extends ScholarlyBaseService {
  ...
  async issueCredential<T extends CredentialSubject>(
    ...
    return this.withTiming('issueCredential', async () => {
      ...
      if (!issuerPassphrase) {
        return failure({ code: 'VALIDATION_ERROR', message: 'issuerPassphrase is required' });
      }
      if (!request.credentialType) {
        return failure({ code: 'VALIDATION_ERROR', message: 'credentialType is required' });
      }
      if (!request.subjectDid) {
        return failure({ code: 'VALIDATION_ERROR', message: 'subjectDid is required' });
      }
      if (!request.subjectData) {
        return failure({ code: 'VALIDATION_ERROR', message: 'subjectData is required' });
      }
      ...
      try {
        ...
        if (!subjectResolution.success) {
          return failure({ code: 'VALIDATION_ERROR', message: `Subject DID not resolvable: ${request.subjectDid}` });
        }
        ...
        return success(signedCredential);
      } catch (error) {
        log.error('Failed to issue credential', error as Error, { tenantId });
        return failure({
          code: 'CREDENTIAL_ISSUANCE_ERROR',
          message: (error as Error).message,
          details: { credentialType: request.credentialType, subjectDid: request.subjectDid }
        });
      }
    });
  }
  ...
  private async signCredential(
    ...
    if (!signResult.success) {
      const failedResult = signResult as { success: false; error: { code: string; message: string } };
      throw new CredentialValidationError('Failed to sign credential', { error: failedResult.error.message });
    }
    ...
    const proof: Proof = {
      ...
      verificationMethod: signData.verificationMethod,
      proofPurpose: 'assertionMethod',
      proofValue: this.crypto.toBase64Url(signData.signature)
    };
  ...
  async verifyCredential(
    ...
    try {
      ...
      log.error('Credential verification failed', error as Error);
  ...
  private validateProperty(value: any, schema: { type?: string; format?: string; enum?: any[] }, fieldName: string): void {
    ...
    if (schema.format === 'date-time' && typeof value === 'string') {
      if (isNaN(Date.parse(value))) {
        throw new Error(`Field ${fieldName} is not a valid date-time`);
      }
    }
  }
  ...
  async getCredentials(
    ...
    return this.withTiming('getCredentials', async () => {
      ...
      if (!holderDid || holderDid.trim() === '') {
        return failure({ code: 'VALIDATION_ERROR', message: 'holderDid is required' });
      }
      ...
      try {
        ...
        log.error('Failed to get credentials', error as Error, { tenantId, holderDid });
  ...
  async revokeCredential(
    ...
    return this.withTiming('revokeCredential', async () => {
      ...
      if (!revokedBy || revokedBy.trim() === '') {
        return failure({ code: 'VALIDATION_ERROR', message: 'revokedBy is required' });
      }
      ...
      try {
        ...
        log.info('Credential revoked', { tenantId, credentialId, reason, revokedBy });

        return success(undefined);
      } catch (error) {
        log.error('Failed to revoke credential', error as Error, { tenantId, credentialId });
        return failure({ code: 'SERVICE_ERROR', message: (error as Error).message });
      }
    });
  }
  ...
  async createPresentation(
    ...
    return this.withTiming('createPresentation', async () => {
      if (!tenantId || tenantId.trim() === '') {
        return failure({ code: 'VALIDATION_ERROR', message: 'tenantId is required' });
      }
      if (!holderDid || holderDid.trim() === '') {
      ...
      if (!holderPassphrase) {
        return failure({ code: 'VALIDATION_ERROR', message: 'holderPassphrase is required' });
      }
      if (!credentials || credentials.length === 0) {
        return failure({ code: 'VALIDATION_ERROR', message: 'At least one credential is required' });
      }
      ...
      try {
        ...
        for (const cred of credentials) {
          if (cred.credentialSubject.id !== holderDid) {
            return failure({ code: 'VALIDATION_ERROR', message: `Credential ${cred.id} does not belong to holder` });
          }
        }
        ...
        if (!signResult.success) {
          return failure(new PresentationError('Failed to sign presentation'));
        }

        const presentationSignData = signResult.data;

        const proof: Proof = {
          type: 'Ed25519Signature2020',
          created: new Date().toISOString(),
          verificationMethod: presentationSignData.verificationMethod,
          proofPurpose: 'authentication',
          proofValue: this.crypto.toBase64Url(presentationSignData.signature),
          challenge: options?.challenge,
          domain: options?.domain
        };
        ...
        return success(signedPresentation);
      } catch (error) {
        log.error('Failed to create presentation', error as Error, { tenantId, holderDid });
        return failure({ code: 'PRESENTATION_ERROR', message: (error as Error).message });
      }
    });
  }
  ...
  async verifyPresentation(
    ...
    try {
      ...
      log.error('Presentation verification failed', error as Error);
  ...
  async processRequestForCredentials(
    ...
    return this.withTiming('processRequestForCredentials', async () => {
      if (!tenantId || tenantId.trim() === '') {
        return failure({ code: 'VALIDATION_ERROR', message: 'tenantId is required' });
      }
      if (!holderDid || holderDid.trim() === '') {
      ...
      if (!request.input_descriptors) {
        return failure({ code: 'VALIDATION_ERROR', message: 'input_descriptors is required' });
      }
      ...
      try {
        ...
        return success({ matchingCredentials, canSatisfy, missingDescriptors });
      } catch (error) {
        log.error('Failed to process presentation request', error as Error, { tenantId, holderDid });
        return failure({ code: 'SERVICE_ERROR', message: (error as Error).message });
      }
    });
  }
...
let instance: VerifiableCredentialsService | null = null;

export function initializeVCService(deps?: any): VerifiableCredentialsService {
  if (!instance) {
    instance = new VerifiableCredentialsService(deps);
    log.info('VerifiableCredentialsService initialized');
  }
  return instance;
}

export function getVCService(): VerifiableCredentialsService {
  if (!instance) {
    throw new Error('VerifiableCredentialsService not initialized. Call initializeVCService() first.');
  }
  return instance;
}
