...
const demoPlans: SubscriptionPlan[] = [
  { id: 'plan_starter_au', name: 'Starter', slug: 'starter', description: 'Perfect for individual learners and small families.', tier: 'starter', currency: 'AUD', prices: [{ billingCycle: 'monthly', amount: 0, displayAmount: 'Free' }], features: [{ key: 'max_learners', label: 'Learners', value: 2 }, { key: 'ask_issy_messages', label: 'Ask Issy messages/month', value: 50 }], entitlements: ['basic_curriculum', 'ask_issy_limited', 'portfolio_basic'], isPublic: true, trialDays: 0, tenantId: 'tenant-demo', createdAt: '2025-01-15T00:00:00Z', updatedAt: '2025-06-01T00:00:00Z' },
  { id: 'plan_family_au', name: 'Family', slug: 'family', description: 'Ideal for homeschool families needing full curriculum support.', tier: 'family', currency: 'AUD', prices: [{ billingCycle: 'monthly', amount: 2900, displayAmount: '$29.00/mo' }, { billingCycle: 'annual', amount: 29000, displayAmount: '$290.00/yr' }], features: [{ key: 'max_learners', label: 'Learners', value: 6 }, { key: 'homeschool_compliance', label: 'NESA/VRQA compliance', value: true }], entitlements: ['full_curriculum', 'ask_issy_standard', 'homeschool_compliance', 'relief_marketplace', 'linguaflow_basic'], isPublic: true, trialDays: 14, tenantId: 'tenant-demo', createdAt: '2025-01-15T00:00:00Z', updatedAt: '2025-06-01T00:00:00Z' },
  { id: 'plan_school_au', name: 'School', slug: 'school', description: 'For registered schools and micro-schools.', tier: 'school', currency: 'AUD', prices: [{ billingCycle: 'annual', amount: 599000, displayAmount: '$5,990.00/yr' }], features: [{ key: 'max_learners', label: 'Learners', value: 500 }, { key: 'eduscrum', label: 'EduScrum orchestration', value: true }], entitlements: ['full_curriculum', 'ask_issy_unlimited', 'eduscrum', 'analytics_advanced', 'lti_integration', 'ssi_credentials'], isPublic: true, trialDays: 30, tenantId: 'tenant-demo', createdAt: '2025-01-15T00:00:00Z', updatedAt: '2025-06-01T00:00:00Z' },
...
const demoSub: Subscription = {
  ...
  entitlements: ['full_curriculum', 'ask_issy_standard', 'portfolio_full', 'homeschool_compliance', 'relief_marketplace', 'linguaflow_basic'],
...
const demoAnalytics: SubscriptionAnalytics = {
  ...
  topFeatureUsage: [{ feature: 'ask_issy', usagePercent: 89.2 }, { feature: 'portfolio', usagePercent: 76.5 }, { feature: 'curriculum_browser', usagePercent: 71.3 }],
};
...
class SubscriptionApiClient {
  ...
  // ENTITLEMENTS (2)
  // =========================================================================
  async getEntitlements(): Promise<EntitlementsResult> { if (DEMO_MODE) return { userId: 'user-demo-001', tenantId: 'tenant-demo', activeSubscriptions: ['sub_demo_family_001'], effectivePlan: 'family', entitlements: [{ key: 'full_curriculum', label: 'Full Australian Curriculum Access', granted: true, source: 'plan_family_au' }, { key: 'ask_issy_standard', label: 'Ask Issy - Standard', granted: true, source: 'plan_family_au', limits: { messagesPerMonth: 500, currentUsage: 127 } }, { key: 'homeschool_compliance', label: 'NESA/VRQA Homeschool Compliance', granted: true, source: 'plan_family_au' }, { key: 'eduscrum', label: 'EduScrum Orchestration', granted: false, requiredPlan: 'school' }], evaluatedAt: now }; return this.sub('GET', '/entitlements'); }
  async checkEntitlement(key: string): Promise<EntitlementCheck> { if (DEMO_MODE) return { entitlement: { key, granted: ['full_curriculum', 'ask_issy_standard', 'homeschool_compliance'].includes(key), label: key }, userId: 'user-demo-001', tenantId: 'tenant-demo', checkedAt: now }; return this.sub('GET', `/entitlements/${key}/check`); }
