...
export const NODE_CATEGORIES_META: Record<NodeCategory, {
  ...
  SOURCE:    { label: 'Source',    color: '#2E75B6', bg: '#E8F0FE', iconName: 'Database' },
  TRANSFORM: { label: 'Transform', color: '#2E8B57', bg: '#E8F5E8', iconName: 'Filter' },
  VISUALISE: { label: 'Visualise', color: '#9B59B6', bg: '#F3E8FD', iconName: 'BarChart3' },
  MODEL:     { label: 'Model',    color: '#D4790E', bg: '#FFF3E0', iconName: 'Brain' },
  ACTION:    { label: 'Action',   color: '#C42B1C', bg: '#FDE8E8', iconName: 'Zap' },
  EDUCATION: { label: 'Education', color: '#1B7A6E', bg: '#E0F7F4', iconName: 'GraduationCap' },
};
...
// ── Category glyphs (zero-dependency, portable) ─────────────────────────
const CATEGORY_GLYPHS: Record<NodeCategory, string> = {
  SOURCE: '◉', TRANSFORM: '⬡', VISUALISE: '📊',
  MODEL: '🧠', ACTION: '⚡', EDUCATION: '🎓',
};
...
const STATUS_COLORS: Record<string, string> = {
  running: '#D4790E', completed: '#2E8B57', failed: '#C42B1C',
  paused: '#9B59B6', pending: '#4A6275', cancelled: '#6A8498',
};

// ── Toolbar Button ──────────────────────────────────────────────────────
function ToolBtn({ label, onClick, accent, disabled, glyph, title }: {
  label: string; onClick: () => void; accent?: string;
  disabled?: boolean; glyph?: string; title?: string;
}) {
  return (
    <button onClick={onClick} disabled={disabled} title={title ?? label}
      style={{
        ...
        border: `1px solid ${accent ?? '#253545'}`, borderRadius: 6,
      ...
      {glyph && <span style={{ fontSize: 14 }}>{glyph}</span>} {label}
    </button>
  );
}
...
function ConfigField({ fieldKey, fieldDef, value, onChange }: {
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', background: '#0F1923', border: '1px solid #253545',
    borderRadius: 6, padding: '6px 10px', color: '#D0D8E0',
  ...
  switch (fieldDef.type) {
    case 'string': return (
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 11, fontWeight: 600, color: '#5A7A90', display: 'block', marginBottom: 4, textTransform: 'uppercase' as const, letterSpacing: 0.5 }}>
        ...
        <input type="text" value={(value as string) ?? fieldDef.defaultValue ?? ''}
          ...
          style={inputStyle} />
        {fieldDef.helpText && <div style={{ fontSize: 10, color: '#4A6275', marginTop: 2 }}>{fieldDef.helpText}</div>}
      </div>
    );
    case 'number': return (
      <div style={{ marginBottom: 12 }}>
    ...
    case 'boolean': return (
      <div style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
        <input type="checkbox" checked={(value as boolean) ?? (fieldDef.defaultValue as boolean) ?? false}
          onChange={e => onChange(fieldKey, e.target.checked)} style={{ accentColor: '#2E75B6' }} />
        <label style={{ fontSize: 12, color: '#8BA4B8' }}>{fieldDef.label}</label>
      </div>
    );
    case 'select': return (
      <div style={{ marginBottom: 12 }}>
    ...
    default: return <div style={{ fontSize: 11, color: '#4A6275' }}>Unsupported: {fieldDef.type}</div>;
  }
}
  ...
  return (
    <div style={{
      ...
      <button onClick={onToggle} style={{
        ...
        <span style={{ fontSize: 14 }}>🤖</span> AI Context
        <span style={{ marginLeft: 'auto', fontSize: 10, color: '#4A7A9B' }}>{isExpanded ? '▼' : '▶'}</span>
      </button>

      {/* Always show contextual role */}
      <div style={{ fontSize: 12, color: '#B0C8DA', lineHeight: 1.5, marginBottom: 8 }}>
      ...
      {isExpanded && (
        <>
          ...
          <div style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: '#5A7A90', marginBottom: 3, textTransform: 'uppercase' as const }}>Data Flow</div>
            <div style={{ fontSize: 11, color: '#8BA4B8', lineHeight: 1.4 }}>
          ...
          {insight.configNotes.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: '#5A7A90', marginBottom: 3, textTransform: 'uppercase' as const }}>Configuration Notes</div>
          ...
          {knowledge?.analogy && (
            <div style={{ fontSize: 11, color: '#7A9AB0', fontStyle: 'italic', lineHeight: 1.5, marginBottom: 8,
...
function SuggestionBanner({ suggestions, onDismiss }: {
  ...
  const levelColors = { info: '#2E75B6', tip: '#D4790E', warning: '#C42B1C' };
    ...
    <div style={{ padding: '0 12px 8px 12px' }}>
      {visible.slice(0, 3).map(s => (
        <div key={s.id} style={{
          ...
          <div style={{ color: '#B0C8DA', lineHeight: 1.4, flex: 1 }}>{s.message}</div>
          <button onClick={() => { setDismissed(prev => new Set([...prev, s.id])); onDismiss(s.id); }}
            style={{ background: 'none', border: 'none', color: '#4A6275', cursor: 'pointer', fontSize: 12, padding: 0, flexShrink: 0 }}>
            ✕
          </button>
        </div>
      ))}
      {visible.length > 3 && (
        <div style={{ fontSize: 10, color: '#4A6275', textAlign: 'center' as const }}>
...
export default function SRCanvasProduction({
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'IBM Plex Sans', 'Segoe UI', sans-serif", background: '#0F1923', color: '#E8ECF0', overflow: 'hidden' }}>
      ...
      {/* ── Left Palette ──────────────────────────────────────────── */}
      <div style={{ width: paletteOpen ? 240 : 44, transition: 'width 0.2s', background: '#162230', borderRight: '1px solid #253545', display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
        <div style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #253545' }}>
          {paletteOpen && <span style={{ fontWeight: 600, fontSize: 13, color: '#8BA4B8', textTransform: 'uppercase' as const, letterSpacing: 1 }}>Nodes</span>}
          ...
          <>
            <div style={{ padding: '8px 8px 4px' }}>
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search nodes..."
                style={{ width: '100%', background: '#0F1923', border: '1px solid #253545', borderRadius: 6, padding: '5px 8px', color: '#D0D8E0', fontSize: 12, outline: 'none' }} />
            </div>
                ...
                return (
                  <div key={catKey} style={{ marginBottom: 4 }}>
                    <button onClick={() => setExpandedCats(p => ({ ...p, [catKey]: !p[catKey] }))}
                      ...
                      {isOpen ? '▼' : '▶'} {CATEGORY_GLYPHS[catKey as NodeCategory]} {cat.label} ({catNodes.length})
                    </button>
                    {isOpen && catNodes.map(nt => (
                      <button key={nt.typeId} onClick={() => addNode(nt.typeId)}
                        style={{ display: 'block', width: '100%', textAlign: 'left' as const, background: '#1A2D3F', border: '1px solid #253545', borderRadius: 6, padding: '8px 10px', marginBottom: 3, cursor: 'pointer', color: '#D0D8E0', fontSize: 12, transition: 'background 0.15s' }}
                        onMouseEnter={e => (e.currentTarget.style.background = '#223A50')} onMouseLeave={e => (e.currentTarget.style.background = '#1A2D3F')}>
                        <div style={{ fontWeight: 600, marginBottom: 2 }}>{nt.label}</div>
                        <div style={{ fontSize: 10, color: '#7A8FA0' }}>{nt.description}</div>
                      </button>
                    ))}
                  </div>
                );
            ...
            {/* Recipes button */}
            <div style={{ padding: 8, borderTop: '1px solid #253545' }}>
              <button onClick={() => setShowRecipes(p => !p)}
                style={{ width: '100%', background: '#1A2D3F', border: '1px solid #253545', borderRadius: 6, padding: '8px 10px', cursor: 'pointer', color: '#8BA4B8', fontSize: 12, fontWeight: 600 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        {/* Toolbar */}
        <div style={{ height: 48, background: '#162230', borderBottom: '1px solid #253545', display: 'flex', alignItems: 'center', padding: '0 12px', gap: 6 }}>
          ...
          <ToolBtn label="Undo" glyph="↶" onClick={undo} disabled={!cmdHistory.current.canUndo} title={cmdHistory.current.undoDescription ? `Undo: ${cmdHistory.current.undoDescription}` : 'Undo (Ctrl+Z)'} />
          <ToolBtn label="Redo" glyph="↷" onClick={redo} disabled={!cmdHistory.current.canRedo} title={cmdHistory.current.redoDescription ? `Redo: ${cmdHistory.current.redoDescription}` : 'Redo (Ctrl+Shift+Z)'} />
          <div style={{ width: 1, height: 24, background: '#253545' }} />
          <ToolBtn label="Save" glyph="💾" onClick={handleSave} accent="#2E75B6" title="Save (Ctrl+S)" />
          <ToolBtn label="Clear" glyph="🗑" onClick={clearCanvas} />
          {selectedNodes.size > 0 && <ToolBtn label={`Delete (${selectedNodes.size})`} glyph="✕" onClick={deleteSelected} accent="#C42B1C" />}
          <ToolBtn label="Fit" glyph="⊞" onClick={() => { const c = svgRef.current?.parentElement; if (c) setViewport(fitToContent(nodes, c.clientWidth, c.clientHeight)); }} />
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: 11, color: '#4A6275', marginRight: 8 }}>{Math.round(viewport.scale * 100)}%</span>
          {/* Execution controls */}
          {runState && (
            <span style={{ fontSize: 12, fontWeight: 600, color: STATUS_COLORS[runState.status] ?? '#ccc', display: 'flex', alignItems: 'center', gap: 4, marginRight: 8 }}>
              {STATUS_GLYPHS[runState.status]} {runState.status.toUpperCase()} ({Math.round(runState.durationMs / 1000)}s)
            </span>
          )}
          {runState?.status === 'paused' && <ToolBtn label="Resume" glyph="▶" onClick={handleResume} accent="#9B59B6" />}
          {(runState?.status === 'running' || runState?.status === 'paused') && <ToolBtn label="Cancel" glyph="■" onClick={handleCancel} accent="#C42B1C" />}
          <ToolBtn label="Execute" glyph="▶" onClick={handleExecute} accent="#2E8B57"
            disabled={nodes.length === 0 || hasErrors || runState?.status === 'running'} />
        </div>

        {/* Recipes dropdown */}
        {showRecipes && (
          <div style={{ background: '#162230', borderBottom: '1px solid #253545', padding: '8px 12px', display: 'flex', gap: 8, flexWrap: 'wrap' as const }}>
            {allRecipes.map(r => (
              <button key={r.id} onClick={() => loadRecipe(r)}
                style={{ background: '#1A2D3F', border: '1px solid #253545', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', color: '#D0D8E0', fontSize: 12, textAlign: 'left' as const }}>
                <div style={{ fontWeight: 600 }}>{r.name}</div>
                <div style={{ fontSize: 10, color: '#7A8FA0' }}>{r.description}</div>
              </button>
            ))}
          </div>
        )}
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
              ...
              {nodes.map(node => {
                ...
                return (
                  <g key={node.id} onMouseDown={e => onNodeDown(e, node.id)}>
                    ...
                    {/* Body */}
                    <rect x={node.x} y={node.y} width={NODE_WIDTH} height={h} rx={8} fill="#1A2D3F"
                    ...
                    <text x={node.x + 10} y={node.y + 23} fontSize={12} fontWeight={600} fill="white" fontFamily="'IBM Plex Sans', sans-serif">{type.label}</text>
                    ...
                    {validationIssues.some(v => v.nodeId === node.id && v.severity === 'error') && !ns && (
                      <circle cx={node.x + NODE_WIDTH - 4} cy={node.y - 4} r={6} fill="#C42B1C" stroke="#0F1923" strokeWidth={2} />
                    )}
                    {/* Input ports */}
                    {type.inputs.map((port, pi) => {
                      ...
                      return (
                        <g key={`in-${port.portId}`} onMouseUp={e => onInPortUp(e, node.id, port.portId, port.dataType)} style={{ cursor: 'crosshair' }}>
                          <circle cx={node.x} cy={py} r={PORT_RADIUS} fill="#1A2D3F" stroke={PORT_COLORS[port.dataType] ?? '#888'} strokeWidth={2} />
                          <text x={node.x + 14} y={py + 4} fontSize={10} fill="#8BA4B8" fontFamily="'IBM Plex Sans', sans-serif">{port.label}</text>
                        </g>
                      );
                    })}
                    {/* Output ports */}
                    {type.outputs.map((port, pi) => {
                      ...
                      return (
                        <g key={`out-${port.portId}`} onMouseDown={e => onOutPortDown(e, node.id, port.portId, port.dataType)} style={{ cursor: 'crosshair' }}>
                          <circle cx={node.x + NODE_WIDTH} cy={py} r={PORT_RADIUS} fill={PORT_COLORS[port.dataType] ?? '#888'} stroke="#1A2D3F" strokeWidth={2} />
                          <text x={node.x + NODE_WIDTH - 14} y={py + 4} fontSize={10} fill="#8BA4B8" textAnchor="end" fontFamily="'IBM Plex Sans', sans-serif">{port.label}</text>
                        </g>
                      );
                    })}
                  </g>
          ...
          {/* Empty state */}
          {nodes.length === 0 && (
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' as const, color: '#4A6275', pointerEvents: 'none' }}>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(22,34,48,0.92)', backdropFilter: 'blur(8px)', borderTop: '1px solid #253545' }}>
            ...
            {/* Status bar */}
            <div style={{ padding: '6px 12px', fontSize: 11, color: '#4A6275', display: 'flex', gap: 12 }}>
              <span>{nodes.length} nodes</span>
              <span>{edges.length} edges</span>
              <span>Zoom: {Math.round(viewport.scale * 100)}%</span>
              {validationIssues.length > 0 && (
                <span style={{ color: hasErrors ? '#C42B1C' : '#D4790E' }}>
      ...
      {selectedNode && selectedType && selectedCat && (
        <div style={{ width: 300, background: '#162230', borderLeft: '1px solid #253545', display: 'flex', flexDirection: 'column', flexShrink: 0, overflowY: 'auto' }}>
          {/* Header */}
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #253545', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontWeight: 600, fontSize: 13, color: selectedCat.color }}>
              {CATEGORY_GLYPHS[selectedType.category]} {selectedType.label}
            </span>
            <button onClick={() => setSelectedNodes(new Set())} style={{ background: 'none', border: 'none', color: '#6A8498', cursor: 'pointer', fontSize: 16 }}>✕</button>
          </div>
          ...
          <div style={{ padding: 16, flex: 1 }}>
            ...
            {selectedKnowledge && (
              <div style={{ marginBottom: 16, padding: '10px 12px', background: '#0F1923', borderRadius: 8, border: '1px solid #253545' }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#5A7A90', marginBottom: 6, textTransform: 'uppercase' as const, letterSpacing: 0.5 }}>What This Node Does</div>
                <div style={{ fontSize: 12, color: '#A0B8CA', lineHeight: 1.5 }}>
            ...
            {nodeState(selectedNode.id) && (
              <div style={{
                ...
                {STATUS_GLYPHS[nodeState(selectedNode.id)!]} {nodeState(selectedNode.id)}
              </div>
            )}
            ...
            {selectedType.inputs.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#5A7A90', marginBottom: 6, textTransform: 'uppercase' as const }}>Inputs</div>
                {selectedType.inputs.map(p => (
                  <div key={p.portId} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, fontSize: 12, color: '#8BA4B8' }}>
            ...
            {selectedType.outputs.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#5A7A90', marginBottom: 6, textTransform: 'uppercase' as const }}>Outputs</div>
            ...
            {/* Config Form */}
            <div style={{ borderTop: '1px solid #253545', paddingTop: 12, marginTop: 4 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#5A7A90', marginBottom: 8, textTransform: 'uppercase' as const, letterSpacing: 0.5 }}>Configuration</div>
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#5A7A90', marginBottom: 4, textTransform: 'uppercase' as const }}>Type ID</div>
              <div style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", color: '#6A8498', background: '#0F1923', padding: '6px 10px', borderRadius: 6 }}>
                {selectedType.typeId}
              </div>
            </div>
          </div>
